                ...
                <div class="space-y-3">
                    @foreach ($notifications as $notification)
                        <form method="POST" action="{{ route('notifications.read', $notification->id) }}">
                            @csrf
                            <button type="submit"
                                class="w-full text-start border rounded-lg p-4 transition {{ $notification->read_at ? 'border-slate-700 text-slate-400' : 'border-indigo-500/50 bg-indigo-500/5 text-white' }}">
